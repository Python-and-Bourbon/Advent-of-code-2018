...
with open('output.txt', 'w') as outp:
    ...
    while gen_number < 300:
        ...
        outp.write(str(gen_number) + ':' + (' ' * (4 - len(str(gen_number)))) + current_gen + '\n')
        out = 0
        for x in range(len(current_gen)):
            if current_gen[x] == '#': out += x-d
        outp.write(str(out)+'\n')

#looking at output.txt the difference between the last ten genertions is 186 each time
#testing if that holds up throughout to avoid running 50,000,000,000 iterations on a macbook
repeated_difference = 186
target_gens = 50000000000

print(out + repeated_difference * (target_gens - gen_number))
